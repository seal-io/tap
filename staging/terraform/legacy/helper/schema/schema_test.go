	...
	cases := []struct {
		...
		*/
...

		{
			...
			Config: map[string]any{
				"ports": []any{1, hcl2shim.UnknownVariableValue},
			},
...
func TestSchemaMap_Validate(t *testing.T) {
	cases := map[string]struct {
		...
		"Bad, should not allow lists to be assigned to string attributes": {
			...
			Config: map[string]any{
				"availability_zone": []any{"foo", "bar", "baz"},
			},
		...
		"Bad, should not allow maps to be assigned to string attributes": {
			...
			Config: map[string]any{
				"availability_zone": map[string]any{"foo": "bar", "baz": "thing"},
			},
		...
		"set with partially computed list and map": {
			...
			Config: map[string]any{
				"outer": []any{
					map[string]any{
						"list": []any{"A", hcl2shim.UnknownVariableValue, "c"},
					},
				},
			},
			Err: false,
