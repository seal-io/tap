...
function seal::format::run() {
  ...
  if [[ ${#path_ignored[@]} -gt 0 ]]; then
    seal::log::debug "pushd ${path}; go list -f \"{{.Dir}}\" ./... | grep -v -E \"$(seal::util::join_array "|" "${path_ignored[@]}")\" | xargs gofmt ${gofmt_opts[*]}; popd"
    [[ "${path}" == "${ROOT_DIR}" ]] || pushd "${path}" >/dev/null 2>&1
    go list -f "{{.Dir}}" ./... | grep -v -E "$(seal::util::join_array "|" "${path_ignored[@]}")" | xargs gofmt "${gofmt_opts[@]}"
    [[ "${path}" == "${ROOT_DIR}" ]] || popd >/dev/null 2>&1
  else
    seal::log::debug "pushd ${path}; go list -f \"{{.Dir}}\" ./... | xargs gofmt ${gofmt_opts[*]}; popd"
    [[ "${path}" == "${ROOT_DIR}" ]] || pushd "${path}" >/dev/null 2>&1
    go list -f "{{.Dir}}" ./... | xargs gofmt "${gofmt_opts[@]}"
    [[ "${path}" == "${ROOT_DIR}" ]] || popd >/dev/null 2>&1
  fi
